 ...
 * Cloudflare's drag-and-drop deploys do NOT compile a /functions folder.
 * A root _worker.js IS supported by drag and drop, so all server-side logic
 * lives here instead.
 *
 * It does two things:
 *   1. Handles POST /api/pay  — signs and forwards the payment to Payfast.
 *   2. Passes every other request through to the normal static site.
 *
 * Merchant credentials run server-side only and are never sent to a browser.
 * Environment variables, if set in Cloudflare Pages, take priority.
 */

const DEFAULT_MERCHANT_ID = "13534520";
const DEFAULT_MERCHANT_KEY = "zzyvsai11dzej";

export default {
  async fetch(request, env) {
    const url = new URL(request.url);

    if (url.pathname === "/api/pay" || url.pathname === "/api/pay/") {
      if (request.method === "POST") {
        try {
          return await handlePay(request, env, url.origin);
        } catch (err) {
          return htmlResponse(
            messagePage(
              "Something went wrong starting the payment",
              "We couldn't hand you over to Payfast just now. Please try again, or contact us on WhatsApp at +27 79 310 8775 and we'll take the payment another way.",
              url.origin + "/payments.html"
            ),
            500
          );
        }
      }
      // Anyone landing here directly (GET) goes back to the payment page.
      return Response.redirect(url.origin + "/payments.html", 302);
    }
...
async function handlePay(request, env, origin) {
  const merchant_id = env.PAYFAST_MERCHANT_ID || DEFAULT_MERCHANT_ID;
  const merchant_key = env.PAYFAST_MERCHANT_KEY || DEFAULT_MERCHANT_KEY;
  ...
  const inputs =
    present.map((k) => `<input type="hidden" name="${k}" value="${escapeAttr(data[k])}">`).join("") +
...
function messagePage(title, body, backUrl) {
  const back = backUrl
    ? `<p style="margin-top:24px"><a href="${backUrl}" style="color:#ec3013">← Back to the payment page</a></p>`
    : "";
...
/* Minimal MD5. Cloudflare's Web Crypto has no MD5, and Payfast signatures
   require it. UTF-8 bytes come from TextEncoder (no legacy unescape). */
function md5(str) {
  ...
  function tb(s) {
    ...
    n[(((bytes.length + 8) >> 6) + 1) * 16 - 2] = bytes.length * 8;
    return n;
  }
  function rh(n) {
    let s = "";
